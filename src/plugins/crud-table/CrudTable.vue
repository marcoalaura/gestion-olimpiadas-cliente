              ...
              >
                <template v-for="(filter, idx) in filters">
                  ...
                  <v-input-autocomplete
                    ...
                    item-value="value"
                    :disabled="filter.disabled"
                    v-on:search="callSearchFunction(search[filter.eventSearch], filter.field)"
                    :clearable="filter.clear"
                    :multiple="filter.multiple"
                    dense
                    :style="!['xs'].includes($vuetify.breakpoint.name) ? 'width: 300px; max-width: 300px' : ''"
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      this.cargarFiltro(this.filters);
  ...
  methods: {
    ...
    valoresBusquedaValidos(objectQuery) {
      let cntParametrosBusqueda = 0;
      for (const key in objectQuery) {
        if (['idOlimpiada', 'limite', 'pagina'].includes(key)) {
          // eslint-disable-next-line no-continue
          continue;
        }
        if (Array.isArray(objectQuery[key]) && objectQuery[key].length === 0) {
          // eslint-disable-next-line no-continue
          continue;
        }
        if (objectQuery[key]) {
          cntParametrosBusqueda++;
        }
      }
      return cntParametrosBusqueda > 0;
    },
    /**
     * @function getData
    ...
    async getData(refresh = false) {
      try {
        if ((this.url || '').includes('null') || (this.url || '').includes('null')) {
          this.setPropsDataTable(false, [], 0);
          return;
        }
        this.loading = true;
        ...
        if (this.filtroAsQuery) {
          ...
          for (const key in this.search) {
            if (typeof this.search[key] !== 'function') {
              searchFiltro[key] = this.search[key];
            }
          }
          query = {
            ...query,
        ...
        if (this.valoresBusquedaValidos(query)) {
          query = {
            ...query,
            page: 1
          };
        ...
        let data;
        if (this.request === 'NORMAL') {
          data = await this.$service.list(this.url, query, filtro);
        } else {
          const url = `${process.env.VUE_APP_BASE_URL}${this.url}?${Object.keys(query)
            .map(key => (query[key] != null ? query[key].toString().length > 0 ? key + '=' + query[key] : null : null))
            .filter(value => value != null)
            .join('&')
            .toString()}&filtro=${filtro}`;
        ...
        if (data?.datos && !data.datos.errores) {
          ...
          if (data) {
            ...
            throw new Error('Existen problemas de conexión. Por favor revise su configuración a Internet.');
          }
        }
        setTimeout(() => {
          this.loaded = true;
    ...
    async callSearchFunction(accion, key) {
      const element = Array.isArray(this.$refs[key]) ? this.$refs[key][0] : this.$refs[key];
      const { internalSearch } = element.$refs.autocomplete;
      await this.delay(100);
      if (accion) {
        await accion(internalSearch);
      }
    },
      ...
      this.$nextTick(() => {
        ...
        newFilters.forEach((filter) => {
          ...
          if (filter.onAutocomplete) {
            this.search[filter.eventSearch] = filter.onAutocomplete;
          }
        });
      });
    },
