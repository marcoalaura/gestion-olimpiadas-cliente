<template>
  <v-dialog v-model="show" persistent content-class="dialog--alert" :max-width="width">
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click.native="cancel()" elevation="0" id="btn-confirm-selected-cancel" class="fs8">{{ textCancel }}</v-btn>
        <v-btn dark color="color1" @click.native="ok()" id="btn-confirm-selected-ok" class="fs8" elevation="0">{{
          textOk
        }}</v-btn>
      </v-card-actions>
    </v-card>
